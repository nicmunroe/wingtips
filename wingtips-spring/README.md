...
uses the `ZipkinHttpTagStrategy` by default to name and tag any created subspans. See 
[Default HTTP Tags](#default_http_tags) in the main readme for details on what tags you get. You can use a different 
tag and naming strategy (and/or tag adapter) if desired by passing it in when constructing the interceptor. 
